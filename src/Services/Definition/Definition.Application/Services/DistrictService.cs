...
namespace Definition.Application.Services
{
    public class DistrictService : IDistrictService
    {
        ...
        private readonly IMongoRepository<District> _districtRepository;
        private readonly IMongoRepository<City> _cityRepository;

        public DistrictService(
            IMongoRepository<District> districtRepository, 
        ...
        {
            City city = await _cityRepository.FirstOrDefaultAsync(c => c.Id == requestModel.CityId);
            if (city == null)
                throw new NotFoundException($"City not found for Id:{requestModel.CityId}");

            await CheckDistrictExist(requestModel);

            District createdDistrict = _mapper.Map<District>(requestModel);
